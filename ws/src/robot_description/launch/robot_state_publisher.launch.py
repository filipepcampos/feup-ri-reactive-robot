...
def launch_setup(context, *args, **kwargs):
    ...
    if extension == "urdf":
        robot_desc_path = os.path.join(
            get_package_share_directory(package_description), "urdf", robot_file
        )
        robot_desc = xacro.process_file(robot_desc_path)
    elif extension == "xacro":
        robot_desc_path = os.path.join(
            get_package_share_directory(package_description), "robot", robot_file
        )
        # We load the XACRO file with ARGUMENTS
        robot_desc = xacro.process_file(
            robot_desc_path, mappings={"reactive_robot_name": reactive_robot_name}
        )
    else:
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [reactive_robot_name_arg, robot_file_arg, OpaqueFunction(function=launch_setup)]
    )
